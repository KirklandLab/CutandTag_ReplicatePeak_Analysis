...
**IMPORTANT**: Always check the `config/sample.csv` with your sample name, BAM file locations, set, mark, and condition before running.
...
+ `set`: sample group for consensus peaks  
  + **{mark}_{condition}**  
  + sets naming for consensus peak files downstream. All samples with the same **Set** name will be combined to generate a consensus peak  
